...
Les sources des tests se trouvent dans src/test/java
    1. Quelles sont les coordonnées du projet ?
groupId : Identifiant unique du groupe ou de l'organisation.
artifactId : Identifiant unique de l'artefact (par exemple, le nom du projet).
version : Version du projet
    1. À quoi sert la propriété `project.build.sourceEncoding` ?
 elle définit l'encodage des fichiers source. Elle est utilisée pour s'assurer que Maven traite correctement les caractères spéciaux et les symboles dans les fichiers source.
    1. Quelles versions des sources et de la JVM sont utilisés ?
maven.compiler.source : Version du langage source Java.
maven.compiler.target : Version de la JVM cible
    1. Quelle version de JUnit est configurée ? À quoi sert la balise `scope` ?
maven.compiler.source : Version du langage source Java.
maven.compiler.target : Version de la JVM cible
    1. À quoi sert la section `pluginManagement` ?
 La section pluginManagement permet de définir des configurations de plugins qui peuvent être héritées par les modules enfants dans un projet multi-modules. Elle permet de centraliser la configuration des plugins.
